...
MVP 검증에서 lint·format·typecheck·build, Vitest 6개, pytest 21개, Playwright 5개를 통과했습니다. Windows Electron에서도 앱 시작과 sandboxed preload, 실제 Python worker 실행 및 잘못된 입력의 실패 보고서 저장, 허용되지 않은 이미지 읽기 거절을 확인했습니다.

Windows의 기존 GPU 환경에서 사용자 체크포인트를 실제 로드하고 기존 검증 PNG 2개를 순차 추론하는 adapter 연결 검사를 통과했습니다. 모델 성능을 판단할 데이터셋 평가 결과는 아닙니다. 실제 라벨이 있는 Cropper 데이터셋을 찾지 못했으므로, 그 데이터로 UI 선택부터 GPU 추론·보고서 저장까지 이어지는 전체 흐름은 아직 검증하지 못했습니다. UI fixture 검사에서 사용하는 가짜 예측은 실제 GPU 검증으로 계산하지 않습니다.

```text
src/backend/     Electron main: 파일 선택, IPC 검증, 평가 프로세스 관리
src/preload/     renderer에 노출하는 이름·타입이 정해진 API
src/shared/      요청·응답·상태·결과 계약
src/frontend/    React 화면과 기능별 상태 관리
python/          기존 ldb-ocr 추론·전처리·디코더·평가 코드 연결
```

Renderer는 파일 시스템이나 Python 프로세스에 직접 접근하지 않습니다. Main은 IPC 발신자와 입력을 검증하고, 선택한 기존 Python 환경에서 평가 worker를 실행합니다. 앱에 서버·원격 실행·학습·작업 큐·DB·라벨 편집 기능은 없습니다.
